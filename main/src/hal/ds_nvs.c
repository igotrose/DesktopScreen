...
NVS_WIFI_INFO_E wifi_config_flag = NVS_WIFO_INFO_NULL;
...
void ds_nvs_save_wifi_info(char *p_ssid, char *p_psw)
{
    ...
    wifi_config_flag = NVS_WIFO_INFO_HAS_SAVE;
...
NVS_WIFI_INFO_E ds_nvs_read_wifi_info(void)
{
    ...
    if (err != ESP_OK)
    {
        ...
        nvs_close(nvs_handle);
        return NVS_WIFO_INFO_NULL;
    }
    wifi_config_flag = flag;
    if (wifi_config_flag == NVS_WIFO_INFO_HAS_SAVE)
    {
...
void ds_nvs_clean_wifi_info(void)
{
    ...
    wifi_config_flag = NVS_WIFO_INFO_NULL;
    ESP_ERROR_CHECK(nvs_set_u8(nvs_handle, "wifi_flag", (uint8_t)wifi_config_flag));
    ESP_ERROR_CHECK(nvs_commit(nvs_handle));
    nvs_close(nvs_handle);
}
...
char *ds_nvs_read_city(void)
{
    ...
    size_t city_len;
    err = nvs_get_str(nvs_handle, "city", NULL, &city_len);
    if (err == ESP_OK)
    {
        char *city = (char *)malloc(city_len);
        err = nvs_get_str(nvs_handle, "city", city, &city_len);
        if (err == ESP_OK)
        {
            printf("city = %s\n", city);
            nvs_close(nvs_handle);
